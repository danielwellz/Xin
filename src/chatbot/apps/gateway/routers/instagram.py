...
from ..utils.security import validate_sha1_signature
...
async def instagram_webhook(
    ...
    try:
        validate_sha1_signature(
            SignatureContext(
                signature=signature, secret=settings.instagram_secret, payload=raw_body
            )
        )
    ...
    message = envelope.to_inbound_message(channel_type=ChannelType.INSTAGRAM)
...
def _parse_payload(payload: dict[str, object]) -> ProviderInboundEnvelope:
    ...
    attachments_seq = attachments if isinstance(attachments, list) else []
